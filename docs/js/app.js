...
function lazyExplain(host, data, place) {
    ...
    fetch(`/api/explain-alert?id=${encodeURIComponent(data.id)}`)
        ...
        .then(d => {
            ...
            if (d && d.explanation) {
                lead.innerHTML = '<div class="hz-plain-label">In plain English · via <a href="https://www.deepseek.com" target="_blank" rel="noopener noreferrer">DeepSeek</a></div>'
    ...
    async function upgradeSection(s) {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                // Credit DeepSeek only once its translation is actually on screen —
                // the regex gloss shown before (or instead, on AI failure) isn't its work.
                const colLabel = el.querySelector('.col-label');
                if (colLabel) colLabel.innerHTML = 'In plain English · via <a href="https://www.deepseek.com" target="_blank" rel="noopener noreferrer">DeepSeek</a>';
